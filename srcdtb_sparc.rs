...
/// SPARC64 mimarisinde DTB adresi için varsayılan adres (UYARI: Gerçek sistemde mimariye özgü yöntemle alınmalıdır!).
pub fn get_dtb_address() -> usize {
    // UYARI: SPARC64 mimarisinde DTB adresi genellikle farklı yöntemlerle (örn. Open Firmware, boot argümanları) belirlenir.
    // Aşağıdaki sadece bir örnektir ve doğru adres gerçek sisteme göre değişir!
    let dtb_address: usize = 0x200000; // Örnek varsayılan adres
    dtb_address
}

#[cfg(not(target_arch = "sparc64"))]
/// SPARC64 dışı mimarilerde (örneğin x86) DTB adresi için varsayılan adres (UYARI: Bu sadece bir örnektir!).
pub fn get_dtb_address() -> usize {
    // UYARI: SPARC64 dışı mimarilerde DTB adresi alımı bu dosyanın kapsamı dışındadır ve
    // farklı bir yöntemle (örneğin, bootloader tarafından sağlanabilir) elde edilmelidir.
    let dtb_address: usize = 0x100000; // Örnek varsayılan adres
    println!("UYARI: SPARC64 dışı mimari için varsayılan DTB adresi kullanılıyor: 0x{:X}. Doğru adresi ayarlayın!", dtb_address);
    dtb_address
}
...
pub fn init() -> Result<(), FdtError>{
    ...
    #[cfg(target_arch = "sparc64")]
    {
        // SPARC64 mimarisinde DTB adresini al (UYARI: Varsayılan adres kullanılıyor!).
