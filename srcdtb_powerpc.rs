#![no_std]
 ...
 /// PowerPC64 mimarisinde DTB adresini r3 yazmacından alır.
 pub fn get_dtb_address() -> usize {
  let dtb_address: usize;
  unsafe {
   // r3 yazmacının değerini dtb_address değişkenine taşı.
   asm!(
    "mr {}, r3", // PowerPC move register komutu: mr hedef, kaynak
    out(reg) dtb_address
   );
  }
  dtb_address
 ...
 pub fn init() -> Result<(), FdtError>{
  ...
  #[cfg(target_arch = "powerpc64")]
  {
   // PowerPC64 mimarisinde DTB adresini al.
  ...
  #[cfg(not(target_arch = "powerpc64"))]
  {
   ...
   println!("UYARI: PowerPC dışı mimari için varsayılan DTB adresi kullanılıyor: 0x{:X}. Doğru adresi ayarlayın!", dtb_address);
  }
