...
// use volatile::Volatile; // Not using volatile in the first example
use bitflags::bitflags;

// AHCI Base Address (Example address, adjust for your system)
const AHCI_BASE: usize = 0xF0000000; // Using the base address from the first example
const AHCI_PORTS_OFFSET: usize = 0x100; // Using the offset from the first example
const SECTOR_SIZE: usize = 512;

// Memory Allocator Trait (Implement a real allocator for your system)
// Removed Allocator trait as it's not used in the first example
...
struct AhciController {
    ...
    pi: Volatile<u32>,  // Port Implemented
    ...
    bohc: Volatile<u32>,
    rsv: [Volatile<u8>; 84],
    ports: [Port; 32], // Maximum 32 port varsayımı
}

#[repr(C)]
struct Port {
    clb: Volatile<u32>,     // Command List Base Address (alt 32 bit)
    clbu: Volatile<u32>,    // Command List Base Address Upper 32 bits
    fb: Volatile<u32>,      // FIS Base Address (alt 32 bit)
    ...
    ssts: Volatile<u32>,    // SATA Status (speed, etc.)
    sctl: Volatile<u32>,    // SATA Control
    serr: Volatile<u32>,    // SATA Error
    sact: Volatile<u32>,    // SATA Active
    ci: Volatile<u32>,      // Command Issued
    sntf: Volatile<u32>,    // SATA Notification
    fbs: Volatile<u32>,     // FIS Based Switching Control
    rsv1: [Volatile<u32>; 9],
    vendor: [Volatile<u8>; 96], // Vendor specific
}

bitflags! {
    struct PortCmdFlags: u32 {
        const START = 1 << 0;  // Start Command Processing
        const FRE    = 1 << 4;  // FIS Receive Enable
        const CR     = 1 << 15; // Command Running
    }
}
...
#[repr(u8)]
enum FisType {
    H2D = 0x27, // Host to Device FIS
    D2H = 0x34, // Device to Host FIS
    DMA = 0x39, // DMA FIS
}
...
struct FisH2D {
    ...
    lba1: u8,           // LBA 8-15
    lba2: u8,           // LBA 16-23
    device: u8,         // Device/Head
    lba3: u8,           // LBA 24-31
    lba4: u8,           // LBA 32-39
    lba5: u8,           // LBA 40-47
    feature_ex: u8,     // Feature Extension
    countl: u8,         // Sector Count Low
    counth: u8,         // Sector Count High
    icc: u8,            // Interface Control Command
    control: u8,        // Control
}

// Command List yapısı (port başına)
...
// FIS yapısı (Frame Information Structure) - İlk örnekteki ile uyumlu hale getirildi
#[repr(C)]
union Fis {
...
// Device to Host FIS yapısı - İlk örnekteki ile uyumlu hale getirildi
#[repr(C)]
struct D2HFis {
    fis_type: u8, // 0x34
    pm_port: u8,
...
// Host to Device FIS yapısı - İlk örnekteki ile uyumlu hale getirildi
#[repr(C)]
struct H2DFis {
    fis_type: u8, // 0x27
    pm_port: u8,
...
// PIO Setup FIS yapısı - İlk örnekteki ile uyumlu hale getirildi
#[repr(C)]
struct PioFis {
    fis_type: u8, // 0x5F
    pm_port: u8,
...
// Set Device Bits FIS yapısı - İlk örnekteki ile uyumlu hale getirildi
#[repr(C)]
struct SdbFis {
    fis_type: u8, // 0xA1
    pm_port: u8,
...
// Volatile wrapper (basitleştirilmiş)
#[repr(transparent)]
...
unsafe fn sata_read(port_index: usize, lba: u64, sectors: u16, buffer: &mut [u8]) -> Result<(), &'static str> {
    ...
    let fis = FisH2D {
        fis_type: FisType::H2D as u8,
        flags: 0x80, // Command FIS
        ...
        feature_ex: 0,
        countl: (sectors & 0xFF) as u8,
        counth: ((sectors >> 8) & 0xFF) as u8,
        icc: 0,
        control: 0,
    };
    let fis_bytes = unsafe { slice::from_raw_parts(&fis as *const FisH2D as *const u8, size_of::<FisH2D>()) };
