...
/// RISC-V mimarisinde DTB adresini a0 yazmacından alır.
pub fn get_dtb_address() -> usize {
    let dtb_address: usize;
    unsafe {
        // a0 yazmacının değerini dtb_address değişkenine taşı.
        asm!("mv {}, a0", out(reg) dtb_address);
    }
    dtb_address
}
...
pub fn init() -> Result<(), FdtError>{
    ...
    #[cfg(target_arch = "riscv64")]
    {
        // RISC-V mimarisinde DTB adresini al.
    ...
    #[cfg(not(target_arch = "riscv64"))]
    {
        ...
        println!("UYARI: RISC-V dışı mimari için varsayılan DTB adresi kullanılıyor: 0x{:X}. Doğru adresi ayarlayın!", dtb_address);
    }
